...
const TimelineEventComfort: React.FC<TimelineEventProps> = ({ event, isLast }) => {
    ...
    <div className={`relative pl-8 pb-8 group ${isLast ? '' : ''}`}>
      ...
      <div className="flex flex-col items-start w-full -mt-1">
        <div className="flex items-center gap-3 mb-1">
          <span className="text-sm font-bold text-stone-400 font-sans tracking-wide dark:text-slate-500 transition-colors duration-700">
            {event.time}
          </span>
          {event.category && (
            <div className="flex items-center gap-1 px-3 py-1 rounded-full bg-[#f4f2ed] border border-[#e6e2d8] text-[10px] font-bold text-stone-600 uppercase tracking-wider dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 transition-colors duration-700">
        ...
        <div className="flex justify-between items-start w-full gap-4">
          <h4 className={`text-lg md:text-xl font-serif font-bold text-stone-800 leading-tight dark:text-slate-100 transition-colors duration-700 ${event.highlight ? 'text-japan-blue underline decoration-japan-blue/20 decoration-2 underline-offset-4 dark:text-sky-400 dark:decoration-sky-500/30' : ''}`}>
            {event.title}
          </h4>
        ...
        <div className="mt-3 text-sm md:text-base text-stone-700 leading-relaxed bg-[#fdfbf7] px-6 py-5 rounded-3xl border border-stone-200/80 w-full md:w-auto shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] hover:border-stone-300 transition-all duration-500 dark:bg-slate-800/50 dark:border-slate-700 dark:text-slate-300">
          {event.desc}
        </div>
...
export default TimelineEventComfort;